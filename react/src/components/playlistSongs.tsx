...
export default function PlaylistSongs() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      axios.get(`${apiUrl}/api/playlists/${id}/songs`),
      axios.get(`${apiUrl}/api/playlists/${id}`)
    ])
      .then(([songsRes, playlistRes]) => {
        setSongs(songsRes.data)
        setPlaylistName(playlistRes.data.name)
      })
    ...
    axios
      ...
      .then(() => {
        ...
        if (currentIndex !== null && songs[currentIndex]?.id === Number(songToDelete)) {
          if (songs.length <= 1) setCurrentIndex(null)
          else if (currentIndex >= songs.length - 1) setCurrentIndex(songs.length - 2)
        }
